...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var gd = document.getElementsByClassName('plotly')[0];

  if (!gd) return;

  gd.on('plotly_restyle', function(eventData) {

    var visible = eventData[0].visible;

    if (!visible) return;

    // Find which trace is TRUE (this tells us which receptor is active)
    var idx = visible.findIndex(v => v === true);

    // FIRST receptor (2b)
    if (idx === 0) {
      document.getElementById('struct_1').style.display = 'block';
      document.getElementById('struct_2').style.display = 'none';
    }

    // SECOND receptor (3a)
    if (idx === 1) {
      document.getElementById('struct_1').style.display = 'none';
      document.getElementById('struct_2').style.display = 'block';
    }
